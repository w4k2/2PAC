...
fig, axx = plt.subplots(9,5,figsize=(15, 12), sharex=True, sharey=True)

for bc_id, bc in enumerate(base_clfs):
    axx[0,bc_id].set_title(base_clfs_names[bc_id])
    axx[8,bc_id].set_xlabel('chunk')
...
plt.savefig('foo.png')
plt.savefig('figures/e2_supp.eps')
plt.savefig('figures/e2_supp.png')
...
fig.subplots_adjust(bottom=.1)
plt.savefig('foo.png')
plt.savefig('figures/e2_bac_supp.eps')
plt.savefig('figures/e2_bac_supp.png')
